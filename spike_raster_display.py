...
def extract_single_neuron_spike_times_between_specific_times(raw_spike_data_path, neuron_identity_data_path, t0, t1, neuronId):
    ...
    adjustedNeuronSpikeTime = [spike_time for spike_time in unadjustedNeuronSpikeTime if t0 < spike_time < t1]
...
def plot_single_neuron_spikes_between_times(spike_time, neuronId, t0, t1, trialNum = None):
    ...
    plt.ylabel('Spike')
    # Set the plot title
    plt.title(f'Neuron {neuronId} Raster Plot{" for Trial " + str(trialNum) if trialNum else ""}')
    # Display the plot
    plt.show()
...
def extract_single_neuron_spiking_rate_across_trials(spike_times, cue_time, neuronId, TIME_BEFORE_CUE = -3, TIME_AFTER_CUE = 3):
    """
...
def sliding_histogram(spikeTimes, begin_time, end_time, bin_width, stride, rate=True):
    ...
    binSpikes = np.asarray([np.sum(np.all([spikeTimes>=binInt[0], spikeTimes<binInt[1]], axis=0)) for binInt in binIntervals]) 
