...
def display_voltage_and_spikes_across_given_time(voltage_data_path, raw_spike_data_path, neuron_identity_data_path, neuron_channel_path, t0, t1, neuronId):
    singleNeuronSpike = extract_single_neuron_spike_times_between_specific_times(raw_spike_data_path, neuron_identity_data_path, t0, t1, neuronId)
    print("singleNeuronSpike: ", singleNeuronSpike)
    plot_single_neuron_spikes_between_times(singleNeuronSpike, neuronId, t0, t1)
...
def display_voltage_and_spikes_given_trial(voltage_data_path, raw_spike_data_path, neuron_identity_data_path, neuron_channel_path, trial_time_info_path, trialNum, neuronId, TIME_BEFORE_CUE=-3, TIME_AFTER_CUE=3):
    singleNeuronSpikeSpecificTrial, cue_time= extract_single_neuron_spike_times_for_specific_trial(raw_spike_data_path, neuron_identity_data_path, trial_time_info_path, trialNum, neuronId, TIME_BEFORE_CUE, TIME_AFTER_CUE)
    print("cue_time: ", cue_time)
    plot_single_neuron_spikes_between_times(singleNeuronSpikeSpecificTrial, neuronId, cue_time + TIME_BEFORE_CUE, cue_time + TIME_AFTER_CUE, trialNum)

    peakChannel = extractPeakChannel(neuronId, neuron_channel_path)
    getDataAndPlot(voltage_data_path, cue_time + TIME_BEFORE_CUE, cue_time + TIME_AFTER_CUE, peakChannel, trialNum)
...
def display_spikes_across_all_trials(raw_spike_data_path, neuron_identity_data_path, trial_time_info_path, neuronId):
    ...
    for neuron in neuronId:
        adjustedSingleNeuronSpikeAcrossAllTrials = extract_single_neuron_spiking_rate_across_trials(spike_times, cue_time, neuron)
        if adjustedSingleNeuronSpikeAcrossAllTrials is None:
            continue
        plot_single_neuron_spikes_across_all_trials(adjustedSingleNeuronSpikeAcrossAllTrials, neuron)

def display_spikes_across_given_trials(listOfNeuronId):
    display_spikes_across_all_trials(raw_spike_data_path, neuron_identity_data_path, trial_time_info_path, listOfNeuronId)
...
def main():
    ...
    listOfNeuronId = list(range(650, 652))
    ...
    display_voltage_and_spikes_given_trial(voltage_data_path, raw_spike_data_path, neuron_identity_data_path, neuron_channel_path, trial_time_info_path, trialNum, neuronId, TIME_BEFORE_CUE, TIME_AFTER_CUE)
