...
def extract_processed_neuron_raster(config):
    """
    ...
    data = np.load(config['raw_spike_data_path'])
    # Has shape (n, 1) where n is the number of spikes

    # Load the neuron identity data from numpy file
    spike_clusters_neuron_identity = np.load(
        config['neuron_identity_data_path'])

    # # Load the template that stores the template ID used per spike
    # templates_used_per_spike_data_path = np.load(
    #     config['templates_used_per_spike_data_path'])

    # Load the magnitude to scale the template by
    amplitudes_data = np.load(config['amplitudes_data_path'])
    assert len(data) == len(spike_clusters_neuron_identity) == len(
        amplitudes_data), "The number of spikes, neuron identity, and template used per spike should be the same"
    ...
    for idx, spike_time in enumerate(data):
        # If the neuronId doesn't exist in the dictionary, add a new dictionary
        if spike_clusters_neuron_identity[idx] not in spikes.keys():
            spikes[spike_clusters_neuron_identity[idx]] = {}
            spikes[spike_clusters_neuron_identity[idx]]['time'] = []
            spikes[spike_clusters_neuron_identity[idx]]['amplitude'] = []

        # Add the spike time to the neuronId's list
        spikes[spike_clusters_neuron_identity[idx]]['time'].append(spike_time)
        spikes[spike_clusters_neuron_identity[idx]]['amplitude'].append(
            amplitudes_data[idx][0])
    # Return the dictionary containing a dictionary where the key is the neuron ID and the have two sub dictionary containing the time of the spikes as well as the templates used for each spike
...
def extract_trial_time_info(config):
    """
    ...
    """
    mat_file_path = config['trial_time_info_path']
    try:
    ...
    assert len(beginning_trial_time) == len(cue_time) == 323
...
def get_neuron_spike_time_template_across_all_trials(spike_times, cue_time, neuronId, TIME_BEFORE_CUE=-3, TIME_AFTER_CUE=3):
    """
    ...
    unadjustedNeuronSpikeTime = np.array(spike_times[neuronId]['time'])
    amplitude_template_scaling_per_spike = np.array(
        spike_times[neuronId]['amplitude'])

    combined_matrix = np.vstack(
        (unadjustedNeuronSpikeTime, amplitude_template_scaling_per_spike))

    print("combined_matrix.shape", combined_matrix.shape)
    assert combined_matrix.shape[0] == 2 and combined_matrix.shape[1] == len(
        unadjustedNeuronSpikeTime)
    # Combine them from (num_spikes, 1) shape into (2, num_spikes) shape

    adjustedNeuronSpikeTime = []
    all_amplitude_template_scaling_per_spike = []

    for cue in cue_time:
        single_trial_spike = []
        single_trial_template_id = []
        for idx, elem in enumerate(combined_matrix[0]):
            spike_time = elem
            amplitude = combined_matrix[1][idx]
            if cue + TIME_BEFORE_CUE < spike_time < cue + TIME_AFTER_CUE:
                single_trial_spike.append(spike_time - cue)
                single_trial_template_id.append(amplitude)
        adjustedNeuronSpikeTime.append(single_trial_spike)
        all_amplitude_template_scaling_per_spike.append(
            single_trial_template_id)
    # each has shape(num_trial, num_spikes_in a specific trial) containing the adjusted times of the spikes
    return adjustedNeuronSpikeTime, all_amplitude_template_scaling_per_spike
